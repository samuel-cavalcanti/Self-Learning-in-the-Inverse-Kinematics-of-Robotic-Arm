...
SOM::SOM(std::string outputFile){
    
    SOM::getSizeNetworkByCSV(outputFile.c_str());
    
    SOM::loadNodes(outputFile.c_str());
...
void SOM::getSizeNetworkByCSV(std::string outputFile){
    std::fstream data;
    ...
    sizeNetwork = std::sqrt(rows);
    
   // std::cout << sizeNetwork << std::endl;
